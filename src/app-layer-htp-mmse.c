                              ...
                              uint8_t direction)
{
    ...
    while (mms->entry_num) {
        ...
        if (htud->tcflags & HTP_FILENAME_SET) {
             ...
             (void)HTPFileClose(hstate, htud, NULL, 0, 0, direction);
        ...
        if (!is_text) {
            ...
            if (!(flags & HTP_FILENAME_SET)) {
                ...
                if (filename) {
                    result = HTPFileOpen(hstate, htud, (uint8_t *)filename, strlen(filename), pos,
                                         data_len>left_len ? left_len : data_len,
                                         id, direction);
                } else {
                    result = HTPFileOpen(hstate, htud, (uint8_t *)"UnknownFile", 11, pos,
                                         data_len>left_len ? left_len : data_len,
                                         id, direction);
                }
    ...
    if (mms->data_left_len) {
        ...
        if (data_len < mms->data_left_len) {
            if (!(htud->tsflags & HTP_DONTSTORE)) {
                result = HTPFileStoreChunk(hstate, htud, pos, data_len, direction);
                if (result == -1) {
                    SCReturnInt(0);
                } else if (result == -2) {
            ...
            if (parsed_len && (!(htud->tsflags & HTP_DONTSTORE))) {
                result = HTPFileStoreChunk(hstate, htud, pos, parsed_len, direction);
                if (result == -1) {
                    SCReturnInt(0);
                } else if (result == -2) {
                    /* we know for sure we're not storing the file */
